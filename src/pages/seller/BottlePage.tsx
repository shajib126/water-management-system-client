...
import { useCreateBottleBySellerMutation, useTotalBottleBySellerQuery, useUpdateBottleBySellerMutation, useWidthdrawBottleBysellerMutation } from "../../redux/api/baseApi";
import { MutationError } from "../../utils/MutationError";
import toast, { Toaster } from "react-hot-toast";
import Loading from "../Loading";

const BottlePage = () => {
    const { isLoading, data } = useTotalBottleBySellerQuery("");
    const [createBottleBySeller] = useCreateBottleBySellerMutation()
    const [updateBottleBySeller] = useUpdateBottleBySellerMutation()
    const [widthdrawBottleBySeller] = useWidthdrawBottleBysellerMutation()
    ...
    const handleEntrySubmit = async(e:FormEvent)=>{
        ...
        const res = await updateBottleBySeller({stock:editStock})
      ...
      const createBottleHandle = async(e:FormEvent)=>{
        ...
        const res = await createBottleBySeller({stock})
        const err = await MutationError(res)
        if(err){
            toast.error(err)
        }else{
      ...
      const widthdrawBottleHandler = async(e:FormEvent)=>{
        ...
        const res = await widthdrawBottleBySeller({stock:widtdraw})
        const err = await MutationError(res)
        if(err){
            toast.error(err)
        }else{
